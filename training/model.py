...
def compute_confidence_targets(pred_logits: torch.Tensor,
    ...
    for day_idx in range(num_pred_days):
        ...
        # Map CE to confidence using 1/(1+CE) instead of exp(-CE)
        # This gives better gradients: CE=0→1.0, CE=1→0.5, CE=5→0.17
        # exp(-CE) with typical CE~3-5 gives tiny values (0.01-0.05) which are hard to learn
        confidence_targets[:, day_idx] = 1.0 / (1.0 + per_sample_ce)
    ...
    for features, prices, _, _ in pbar:
        ...
        with autocast(enabled=use_amp):
            ...
            if pred_mode == 'regression':
                ...
                # Normalize both to same scale to prevent magnitude mismatch
                expected_values = compute_expected_value(pred, bin_edges)  # (batch, num_pred_days)

                # Normalize both tensors to zero mean, unit variance for scale-invariant comparison
                ev_mean, ev_std = expected_values.mean(), expected_values.std() + 1e-8
                prices_mean, prices_std = prices.mean(), prices.std() + 1e-8
                expected_values_norm = (expected_values - ev_mean) / ev_std
                prices_norm = (prices - prices_mean) / prices_std
                expected_value_loss = F.mse_loss(expected_values_norm, prices_norm)
