...
def softmax_weights(predictions, beta=150.0):
    """
    Compute softmax weights from a list/array of predictions.
    The sum of the weights is 1.
    A higher beta intensifies the bias toward higher predictions.
    """
    exps = np.exp(beta * predictions)
    exps = exps / exps.sum()
    exps = np.nan_to_num(exps, nan=1.0)
    return exps