...
class stock_inference:
    def __init__(self, model_pths, s_pth, p_pth, sectors_pth, device='cuda', start_date=(2024, 5, 1), 
        ...
        self.rank_profits = []
        self.bool_profit = []
        self.use_softmax_buy_rate = softmax
    ...
    def get_top_q_buy(self, top_n, date, show=True, low=False, entropy=True, steps=1, e_high=True,
        ...
        if buy_sell:
            ...
            if low:
                ...
                b_p = b_p[-top_n:]
        if self.use_softmax_buy_rate:
            #print(type(b_p))
            #print(type(b_p))
            a = [item.cpu() for item in b_p]
            self.softmax_buy_weights = softmax_weights(np.array(a))
            self.softmax_buy_weights.tolist()
            #print(type(b_p))
        ...
        if entropy:
            if e_high:
                ...
                n = 10
                low_entropy_values = sorted_entropy_values[:n]
                inds = [entropy_values.index(item) for item in low_entropy_values]
                n_companies = [n_companies[item] for item in inds]
    ...
    def get_date_profit(self, date, companies, low=False, offset=True,counter=1):
        ...
        if do:
            ...
            for i in range(len(companies)):
                if self.use_softmax_buy_rate:
                    print(f'{companies[i]}: {profit[i]:.4f} buy rate: {self.softmax_buy_weights[i]:.4f} buy: {buy_prices[i]:4f} sell: {sell_prices[i]:4f} pos profit avg: {self.rank_profits_acc[i]/(counter+1):.4f}')
                else:
                    print(f'{companies[i]}: {profit[i]:.4f} buy: {buy_prices[i]:4f} sell: {sell_prices[i]:4f} pos profit avg: {self.rank_profits_acc[i]/(counter+1):.4f}')
            #print(profit)
        if self.use_softmax_buy_rate:
            p = 0
            for i in range(len(companies)):
                p += self.softmax_buy_weights[i]*profit[i]
            profit = p
        else:
